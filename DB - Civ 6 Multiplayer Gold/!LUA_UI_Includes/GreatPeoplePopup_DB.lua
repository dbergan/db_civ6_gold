...
-- DB - don't add other players' progress toward great people to the panel [BM: Remove Foreign Info]
			elseif not GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") then
-- /DB
...
-- reverse data so that the most recent recruits are on top
	local len = #data.Timeline
	for i = len - 1, 1, -1 do
		data.Timeline[len] = table.remove(data.Timeline, i)
	end
-- /DB
	for i, kPerson:table in ipairs(data.Timeline) do	
...
-- DB - my version comments this out because otherwise it overrides what Heroes mode adds
--[[
...
-- DB - the wildcard include must be disabled to stop an infinite regression
