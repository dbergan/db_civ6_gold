...
DB_AddAvailableCivic_LeaderIconIM = {}
DB_RealizeCurrentCivic_LeaderIconIM = {}
...
-- print('DBTEST', 'AddAvailableCivic')
	...
	if not GameConfiguration.GetValue("DB_LEARN_FROM_OTHER_CIVS") or kData == nil or kData.CivicType == nil or ListInstance == nil then return ListInstance end

	if DB_AddAvailableCivic_LeaderIconIM[kData.CivicType] ~= nil then
		DB_AddAvailableCivic_LeaderIconIM[kData.CivicType]:ResetInstances()
		DB_AddAvailableCivic_LeaderIconIM[kData.CivicType] = nil
	end
	PlaceLeaderIcons(DB_AddAvailableCivic_LeaderIconIM, ListInstance.DB_NameStack, ListInstance, kData.CivicType, GameInfo.Civics[kData.CivicType].Index, "C", -5, -30)
...
-- print('DBTEST', 'RealizeCurrentCivic')
	...
	if DB_RealizeCurrentCivic_LeaderIconIM ~= nil then
		for _, v in pairs(DB_RealizeCurrentCivic_LeaderIconIM) do
			v:ResetInstances()
			v = nil
		end
	end
	PlaceLeaderIcons(DB_RealizeCurrentCivic_LeaderIconIM, kControl.TitleStack, kControl, kData.CivicType, GameInfo.Civics[kData.CivicType].Index, "C", -5, -15)
end

function ResetCivicLeaderIcons()
-- print('DBTEST', '', 'ResetCivicLeaderIcons')
	if DB_AddAvailableCivic_LeaderIconIM ~= nil then
		for _, v in pairs(DB_AddAvailableCivic_LeaderIconIM) do
			v:ResetInstances()
			v = nil
		end
		DB_AddAvailableCivic_LeaderIconIM = nil
		DB_AddAvailableCivic_LeaderIconIM = {}
	end
	if DB_RealizeCurrentCivic_LeaderIconIM ~= nil then
		for _, v in pairs(DB_RealizeCurrentCivic_LeaderIconIM) do
		...
		DB_RealizeCurrentCivic_LeaderIconIM = nil
		DB_RealizeCurrentCivic_LeaderIconIM = {}
	end
end
