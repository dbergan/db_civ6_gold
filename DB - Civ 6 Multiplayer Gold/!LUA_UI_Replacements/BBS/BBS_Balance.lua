------------------------------------------------------------------------------
-- DB 2021-06-29  
-- This file is a copy of BBS_Balance from the BBS mod
-- My modification was to replace all forms of "DONTMATCH_CIVILIZATION_INCA" with "DONTMATCH_DONTMATCH_CIVILIZATION_INCA" and "DONTMATCH_LEADER_PACHACUTI" with "DONTMATCH_DONTMATCH_LEADER_PACHACUTI" in the if statements that would present these guys with start biases
-- This file will be imported when the BBS mod is active and RemoveCivLeaderTraits game option is enabled
------------------------------------------------------------------------------
...
local COASTAL_CIVS = {"DONTMATCH_LEADER_VICTORIA", "DONTMATCH_LEADER_HOJO", "DONTMATCH_LEADER_DIDO"};  -- Use a IsCoastalCiv ? Like IsTundraCiv 
local COASTAL_LEADERS = {"DONTMATCH_LEADER_VICTORIA", "DONTMATCH_LEADER_HOJO", "DONTMATCH_LEADER_DIDO"};
	...
	if currentTurn == GameConfiguration.GetStartTurn() then
		...
		for i = 1, major_count do
			...
			if (majList[i] ~= nil) then
				if(majList[i].leader ~= "LEADER_SPECTATOR" and majList[i].leader ~= "DONTMATCH_LEADER_LADY_SIX_SKY") then
		...
		for i = 1, major_count do
			-- Added Spectator mod handling if a major player isn't detected
			if (majList[i] ~= nil) then
				if(majList[i].leader ~= "LEADER_SPECTATOR"    and majList[i].leader ~= "DONTMATCH_LEADER_PACHACUTI"  ) then
				...
				if(majList[i].leader == "DONTMATCH_LEADER_PACHACUTI" and  (majList[i].impassable_start + majList[i].impassable_inner + majList[i].impassable_outer) < 2)  then
						__Debug("Mountain Terraforming Start X: ", majList[i].plotX, "Start Y: ", majList[i].plotY, "Player: ",i," ",majList[i].leader, majList[i].civ);
		...
		for i = 1, major_count do
			...
			if (majList[i] ~= nil) then
				if(majList[i].leader ~= "LEADER_SPECTATOR"  ) then
					if ( ( (majList[i].impassable_start + majList[i].water_start ) > 4) and majList[i].leader ~= "DONTMATCH_LEADER_PACHACUTI"  ) then
						-- Check for Walled-in
		...
		for i = 1, major_count do
			...
			if (majList[i] == nil or majList[i].leader == "LEADER_SPECTATOR"  ) then
				...
				if (majList[i].civ == "DONTMATCH_CIVILIZATION_MALI" ) then
					majList[i].food_spawn_start = majList[i].food_spawn_start + tempEval[12] * 1.5
					elseif (majList[i].civ == "DONTMATCH_CIVILIZATION_CANADA" ) then
					...
					elseif (majList[i].civ == "DONTMATCH_CIVILIZATION_RUSSIA" ) then
					...
					elseif (majList[i].civ == "DONTMATCH_CIVILIZATION_MAORI" ) then
		...
		for i = 1, major_count do
			if (majList[i] ~= nil) then
				...
				-- But is the leader biased toward Mountains? 
							if (((majList[i].civ == "DONTMATCH_CIVILIZATION_INCA") and (math.floor(avgFood  - majList[i].food_spawn_start) < 4)) or ((majList[i].civ == "DONTMATCH_CIVILIZATION_MAPUCHE") and (math.floor(avgFood + iBalancingTwo - majList[i].food_spawn_start) < 4))) then
				...
				-- Well it has to be a shitty start then....
							...
							local count_2 = 0;
							for j = 1, math.max(math.floor(avgFood  + 1 - majList[i].food_spawn_start), minFood - majList[i].food_spawn_start,5) do
								if (majList[i].civ == "DONTMATCH_CIVILIZATION_MALI" and majList[i].desert_start > 0) then
									...
									elseif (majList[i].civ == "DONTMATCH_CIVILIZATION_RUSSIA" or majList[i].civ == "DONTMATCH_CIVILIZATION_CANADA" and majList[i].snow_start > 2) then
      ...
      ---- END of Coastal balancing ------
		...
		for i = 1, major_count do
			...
			if (majList[i] == nil or majList[i].leader == "LEADER_SPECTATOR") then
				...
				if (majList[i].civ == "DONTMATCH_CIVILIZATION_MAORI" ) then
			...
			if (majList[i] ~= nil) then
				if(majList[i].leader ~= "LEADER_SPECTATOR") then
					...
					if ( missing_amount > 0.75 ) then
						...
						if (majList[i].civ == "DONTMATCH_CIVILIZATION_RUSSIA" or majList[i].civ == "DONTMATCH_CIVILIZATION_CANADA" ) and majList[i].tundra_start > 3 then
							Terraforming_Best(Map.GetPlot(majList[i].plotX,majList[i].plotY), missing_amount, max_best_tile_1, max_best_tile_2, avg_best_ring_1 ,avg_best_ring_2 ,majList[i].best_tile, majList[i].bestTiles1Ring1_index, majList[i].best_tile_2, majList[i].bestTiles1Ring2_index,majList[i].best_tile_3,majList[i].bestTiles1Ring3_index,majList[i].best_tile_inner,majList[i].bestTiles2Ring1_index,majList[i].best_tile_inner_2,majList[i].bestTiles2Ring2_index, 1, false);
							elseif (majList[i].civ == "DONTMATCH_CIVILIZATION_MALI")  and majList[i].desert_start > 3 then
							Terraforming_Best(Map.GetPlot(majList[i].plotX,majList[i].plotY), missing_amount, max_best_tile_1, max_best_tile_2,avg_best_ring_1 ,avg_best_ring_2,majList[i].best_tile, majList[i].bestTiles1Ring1_index, majList[i].best_tile_2, majList[i].bestTiles1Ring2_index,majList[i].best_tile_3,majList[i].bestTiles1Ring3_index,majList[i].best_tile_inner,majList[i].bestTiles2Ring1_index,majList[i].best_tile_inner_2,majList[i].bestTiles2Ring2_index, 2, false);
							else
							Terraforming_Best(Map.GetPlot(majList[i].plotX,majList[i].plotY), missing_amount, max_best_tile_1, max_best_tile_2, avg_best_ring_1 ,avg_best_ring_2,majList[i].best_tile, majList[i].bestTiles1Ring1_index, majList[i].best_tile_2, majList[i].bestTiles1Ring2_index,majList[i].best_tile_3,majList[i].bestTiles1Ring3_index,majList[i].best_tile_inner,majList[i].bestTiles2Ring1_index,majList[i].best_tile_inner_2,majList[i].bestTiles2Ring2_index, 0, false);
						end
      ...
      --]]
		...
		for i = 1, major_count do
			...
			if (majList[i] == nil or majList[i].leader == "LEADER_SPECTATOR"  ) then
				...
				if (majList[i].civ == "DONTMATCH_CIVILIZATION_RUSSIA" or majList[i].civ == "DONTMATCH_CIVILIZATION_CANADA" ) and tempEval[11] > 4 then
				...
				if (majList[i].civ == "DONTMATCH_CIVILIZATION_MALI" ) and tempEval[12] > 4 then
				-- Mali on Desert
				--	Ring 1
				majList[i].best_tile = tempEval[24]+1.5;
				majList[i].best_tile_2 = tempEval[25]+1.5;
				...
				if (majList[i].civ == "DONTMATCH_CIVILIZATION_MAORI" ) and tempEval[14] > 4 then
				-- Maori if on water
				--	Ring 1 like a 2:2
...
function Terraforming_Best_Refresh(majList,major_count,step,bHighRoll)
		...
		for i = 1, major_count do
			if (majList[i] == nil or majList[i].leader == "LEADER_SPECTATOR"  ) then
				...
				if (majList[i].civ == "DONTMATCH_CIVILIZATION_RUSSIA" or majList[i].civ == "DONTMATCH_CIVILIZATION_CANADA" ) and _tempEval[11] > 4 then
				-- Russia/Canada on Tundra
				--	Ring 1
				...
				if (majList[i].civ == "DONTMATCH_CIVILIZATION_MALI" ) and _tempEval[12] > 4 then
				-- Mali on Desert
				--	Ring 1
				...
				if (majList[i].civ == "DONTMATCH_CIVILIZATION_MAORI" ) and _tempEval[14] > 4 then
				-- Maori if on water
				--	Ring 1 like a 2:2
