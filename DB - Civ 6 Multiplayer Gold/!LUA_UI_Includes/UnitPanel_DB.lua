-- From G:\SteamLibrary\steamapps\common\Sid Meier's Civilization VI\Base\Assets\UI\Panels\UnitPanel.lua
-- edits marked with DB

-- DB - compatibility with Builder / Military Engineer Lag Fix
useOriginalUnitFunction = true
-- /DB
...
-- DB - compatibility with Builder / Military Engineer Lag Fix
							if Modding.IsModActive('a37c6eda-1c44-4c61-b66f-c5048af62b88') and useOriginalUnitFunction == false then
								-- Changed last parameters from true, [default value] to false, false
...
-- DB
				if sDesc ~= nil and sDesc ~= "" then
					if GameInfo.UnitAbilities[ability].Name ~= nil then
...
-- DB
			local ActualCombat = true
			local ReturnFire = true
			local DefStr = defenderTotal - attackerTotal
			local DefHealth = m_combatResults[CombatResultParameters.DEFENDER][CombatResultParameters.MAX_HIT_POINTS] - m_combatResults[CombatResultParameters.DEFENDER][CombatResultParameters.FINAL_DAMAGE_TO] + m_combatResults[CombatResultParameters.DEFENDER][CombatResultParameters.DAMAGE_TO]
			local AttStr = attackerTotal - defenderTotal
			local AttHealth = m_combatResults[CombatResultParameters.ATTACKER][CombatResultParameters.MAX_HIT_POINTS] - m_combatResults[CombatResultParameters.ATTACKER][CombatResultParameters.FINAL_DAMAGE_TO] + m_combatResults[CombatResultParameters.ATTACKER][CombatResultParameters.DAMAGE_TO]
			local WallMaxHealth = m_combatResults[CombatResultParameters.DEFENDER][CombatResultParameters.MAX_DEFENSE_HIT_POINTS]
			local WallHealth = m_combatResults[CombatResultParameters.DEFENDER][CombatResultParameters.MAX_DEFENSE_HIT_POINTS] - m_combatResults[CombatResultParameters.DEFENDER][CombatResultParameters.FINAL_DEFENSE_DAMAGE_TO] + m_combatResults[CombatResultParameters.DEFENDER][CombatResultParameters.DEFENSE_DAMAGE_TO]
			
			if m_combatResults[CombatResultParameters.DEFENDER][CombatResultParameters.ID].type == 1 then
				local DefUnit = UnitManager.GetUnit(m_combatResults[CombatResultParameters.DEFENDER][CombatResultParameters.ID].player, m_combatResults[CombatResultParameters.DEFENDER][CombatResultParameters.ID].id)
				if DefUnit:GetCombat() == 0 and DefUnit:GetReligiousStrength() == 0 then
					ActualCombat = false
				end
			end

			if combatType ~= CombatTypes.MELEE and combatType ~= CombatTypes.RELIGIOUS then
				ReturnFire = false
			end

			Controls.DB_TargetDamagePossibilities:SetText('')
			Controls.DB_DamagePossibilities:SetText('')
			if ActualCombat then
				Controls.DB_TargetDamagePossibilities:SetText(DB_DamagePossibilities(AttStr, DefHealth, WallMaxHealth, WallHealth) .. Locale.Lookup("LOC_GOLD_LABEL"))
				
				if ReturnFire then
					Controls.DB_DamagePossibilities:SetText(DB_DamagePossibilities(DefStr, AttHealth, 0, 0) .. Locale.Lookup("LOC_GOLD_LABEL"))
				end
			end
-- /DB
		else
...
-- DB - compatibility with Builder / Military Engineer Lag Fix
    elseif Modding.IsModActive('a37c6eda-1c44-4c61-b66f-c5048af62b88') and uiMsg == KeyEvents.KeyUp and uiKey == Keys.J then
		useOriginalUnitFunction = not useOriginalUnitFunction
			
		if useOriginalUnitFunction == true then
			NotificationManager.SendNotification(Players[Game.GetLocalPlayer()], NotificationTypes.USER_DEFINED_1, "Lag fix: stopped")
		else
			NotificationManager.SendNotification(Players[Game.GetLocalPlayer()], NotificationTypes.USER_DEFINED_1, "Lag fix: active")
		end
...
function DB_DamagePossibilities(Diff, Health, WallMaxHealth, WallHealth)
	local DmgArray = {}
	local Damage = 1
	for a = 24, 35 do
		Damage = a * 2.6553 ^ (Diff/25)
		if WallMaxHealth > 0 then
			local WallPct = 1 - (WallHealth / WallMaxHealth)
			Damage = WallPct * Damage
		end
		if Damage < 1 then
			Damage = 1
		end
		Damage = DB_Round(Damage)
		table.insert(DmgArray, Health - Damage)
	end
	local DPString = ''
	for _, a in ipairs(DmgArray) do
		if a <= 0 then
			DPString = DPString .. '0 '
		else
			DPString = DPString .. tostring(a) .. ' '
		end
	end
	return DPString
end

-- from https://stackoverflow.com/a/18313481/1997792
function DB_Round(a)
...
function DB_Combat_DebugPrintAllCombatResult(CombatResult)
	...
	print("CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_INTERCEPTOR]", CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_INTERCEPTOR])
	DB_SuperPrint(CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_INTERCEPTOR])
	print("CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_ANTI_AIR]", CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_ANTI_AIR])
	DB_SuperPrint(CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_ANTI_AIR])
	print("CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_HEALTH]", CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_HEALTH])
	DB_SuperPrint(CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_HEALTH])
	print("CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_TERRAIN]", CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_TERRAIN])
	DB_SuperPrint(CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_TERRAIN])
	print("CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_OPPONENT]", CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_OPPONENT])
	DB_SuperPrint(CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_OPPONENT])
	print("CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_MODIFIER]", CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_MODIFIER])
	DB_SuperPrint(CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_MODIFIER])
	print("CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_ASSIST]", CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_ASSIST])
	DB_SuperPrint(CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_ASSIST])
	print("CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_PROMOTION]", CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_PROMOTION])
	DB_SuperPrint(CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_PROMOTION])
	print("CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_DEFENSES]", CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_DEFENSES])
	DB_SuperPrint(CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_DEFENSES])
	print("CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_RESOURCES]", CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_RESOURCES])
	DB_SuperPrint(CombatResult[CombatResultParameters.ATTACKER][CombatResultParameters.PREVIEW_TEXT_RESOURCES])
	print(" ")
	...
	print("CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_INTERCEPTOR]", CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_INTERCEPTOR])
	DB_SuperPrint(CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_INTERCEPTOR])
	print("CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_ANTI_AIR]", CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_ANTI_AIR])
	DB_SuperPrint(CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_ANTI_AIR])
	print("CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_HEALTH]", CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_HEALTH])
	DB_SuperPrint(CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_HEALTH])
	print("CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_TERRAIN]", CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_TERRAIN])
	DB_SuperPrint(CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_TERRAIN])
	print("CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_OPPONENT]", CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_OPPONENT])
	DB_SuperPrint(CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_OPPONENT])
	print("CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_MODIFIER]", CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_MODIFIER])
	DB_SuperPrint(CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_MODIFIER])
	print("CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_ASSIST]", CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_ASSIST])
	DB_SuperPrint(CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_ASSIST])
	print("CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_PROMOTION]", CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_PROMOTION])
	DB_SuperPrint(CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_PROMOTION])
	print("CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_DEFENSES]", CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_DEFENSES])
	DB_SuperPrint(CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_DEFENSES])
	print("CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_RESOURCES]", CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_RESOURCES])
	DB_SuperPrint(CombatResult[CombatResultParameters.DEFENDER][CombatResultParameters.PREVIEW_TEXT_RESOURCES])
	print(" ")
...
function DB_SuperPrint(a, indents)
