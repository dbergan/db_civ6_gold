...
-- DateCreated: 5/10/2019 11:30:08 PM
--------------------------------------------------------------
-- ===========================================================================
--	Unit Flag Manager
...
--	VARIABLES
-- ===========================================================================
-- @ Zur13 MODIFIER ARGUMENTS CACHE
local zumGrantedAbilities = {};
do
	for argDesc in GameInfo.ModifierArguments() do 
		if string.match( argDesc.ModifierId, "_GRANT_ABILITY" ) and argDesc.Name == "AbilityType" then
			-- ** Natural Wonder modifiers and some more
			zumGrantedAbilities[ argDesc.Value ] = argDesc;
		elseif string.match( argDesc.ModifierId, "_UNIT" ) and string.match( argDesc.ModifierId, "_TRAIN" ) and argDesc.Name == "AbilityType" then
			-- ** building trained modifiers
			zumGrantedAbilities[ argDesc.Value ] = argDesc;
		end
	end 
end
...
-- @ Zur13 flag hover
local Zum_ShowPathsOnHover = true;
local Zum_IsPathShown = false;
-- @ Zur13 flag hover
...
-- DB
		...
		if DB_HasMetAllMajors() then
			BasicsString = BasicsString .. ' (' .. pUnit:GetX() .. ',' .. pUnit:GetY() .. ')'
		end
		BasicsString = BasicsString .. '[NEWLINE]'
		if pUnit:GetDamage() > 0 then
			local health = pUnit:GetMaxDamage() - pUnit:GetDamage()
			BasicsString = BasicsString .. health .. '[ICON_Damaged] '
		end
		if pUnit:GetRangedCombat() > 0 then
			BasicsString = BasicsString .. pUnit:GetRangedCombat() .. '[ICON_Ranged] ' .. pUnit:GetRange() .. '[ICON_Range] '
		end
		if pUnit:GetBombardCombat() > 0 then
			BasicsString = BasicsString .. pUnit:GetBombardCombat() .. '[ICON_Bombard] ' .. pUnit:GetRange() .. '[ICON_Range] '
		end
		if pUnit:GetReligiousStrength() > 0 then
			BasicsString = BasicsString .. pUnit:GetReligiousStrength() .. '[ICON_Religion] '
		else
			BasicsString = BasicsString .. pUnit:GetCombat() .. '[ICON_Strength] '
		end
		BasicsString = BasicsString .. pUnit:GetMaxMoves() .. '[ICON_Movement] '
		if pUnit:GetAntiAirCombat() > 0 then
			BasicsString = BasicsString .. pUnit:GetAntiAirCombat() .. '[ICON_AntiAir_Large] '
		end
		if pUnit:HasHiddenVisibility() then
			BasicsString = BasicsString .. '*hidden* '
		end
		if pUnit:IgnoresZOC() then
			BasicsString = BasicsString .. '*elusive* '
		end
		if pUnit:IsEmbarked() then
			BasicsString = BasicsString .. '*embarked* '
		end
		...
		if pUnit:GetExperience() ~= nil and pUnit:GetExperience():GetExperiencePoints() ~= nil and pUnit:GetExperience():GetExperiencePoints() > 0 then
			XPString = pUnit:GetExperience():GetExperiencePoints() .. 'xp (next level at ' .. pUnit:GetExperience():GetExperienceForNextLevel() .. 'xp)'
		end
...
-- print('', pUnit:GetName(), Ability, GameInfo.UnitAbilities[Ability], GameInfo.UnitAbilities[Ability].Index, GameInfo.UnitAbilities[Ability].Name, GameInfo.UnitAbilities[Ability].Description)
		...
		nameString = nameString .. BasicsString

		if XPString ~= '' then
			nameString = nameString .. '[NEWLINE]' .. XPString
		end

		if AbilityString ~= '' then
			AbilityString = Locale.Lookup('{LOC_GOLD_UNIT_PROMOTIONS_AND_ABILITIES_HEADER}') .. AbilityString
		end
	...
	if unitList ~= nil then
		...
		for _, pUnit in ipairs(unitList) do
			...
			if ( flag ~= nil and flag.m_eVisibility == RevealedState.VISIBLE ) then
				...
				ZumUpdateTopBar( unitOwner, unitID ); -- Zur13
			end
		end
	end
end
		...
		if (pPlayer ~= nil) then
			...
			if (pUnit ~= nil) then
				...
				if (flag ~= nil) then
					...
					flag:UpdatePromotions(); -- Zur13 remove promotion available indicator
					ZumUpdateTopBar( playerID, unitID ); -- Zur13
				end
			end
		end
	...
	if (pPlayer ~= nil) then
		...
		if (pUnit ~= nil) then
			...
			if (flagInstance ~= nil) then
				...
				ZumUpdateTopBar( player, unitID ); -- Zur13
			end
		end
	end
end
...
-- ++ Barbarian Clans mode compatibility

local isBarbariansClansModActive:boolean  = Modding.IsModActive("19ED1A36-D744-4A58-8F8B-0376C2BA86E5"); -- Zur13: 
...
function UnitFlag.UpdatePromotions( self )
	...
	if(pUnit ~= nil and isBarbariansClansModActive)then -- Zur13: added isBarbariansClansModActive check
...
function UnitFlag.UpdateName( self )
	...
	if( not isBarbariansClansModActive ) then return; end; -- Zur13: added isBarbariansClansModActive check
	local localPlayerID : number = Game.GetLocalPlayer();
	if (localPlayerID == -1) then
		return;
	end
...
-- DB
				nameString = nameString .. "[NEWLINE][NEWLINE]Barbarian Clan: " .. Locale.Lookup(pBarbTribe.TribeDisplayName);
-- /DB
...
-- ZUM mod related stuff

local CONST_COLOR_HIDE = UI.GetColorValueFromHexLiteral(0x00FFFFFF);
local PANEL_ON_TOP_OFFSET_Y = -8;
local PANEL_ON_BOTTOM_OFFSET_Y = 45;

include( "ZUM_UnitModifiers" );
...
--************************************************************
function ZumUpdateTopBar( playerID : number, unitID : number )
	if playerID == -1 then
		return;
	end
	...
	local SETTINGS_NAME_PREFIX = "OTHER";
	local idLocalPlayer = Game.GetLocalPlayer();
	if playerID == idLocalPlayer then
		SETTINGS_NAME_PREFIX = "MY";
	end

	--local playerFlagInstances = m_UnitFlagInstances[ playerID ];
	--for id, flag in pairs( playerFlagInstances ) do
	--end -- for id, flag 

	local flag = GetUnitFlag( playerID, unitID );

	if flag ~= nil and flag:GetUnit() ~= nil then
		local pUnit : table = flag:GetUnit();
		local unitInfo = GameInfo.Units[ pUnit:GetType() ];
		
		
		if ExposedMembers.ZUM.IsPanelEnabled and ExposedMembers.ZUM.IsPanelEnabled() == false then
			flag.m_Instance.ZUM_DetailsBar:SetHide( true );
			return;
		else
			flag.m_Instance.ZUM_DetailsBar:SetHide( false );
		end

		flag.m_Instance.ZUM_DetailsBar:SetVoid1( playerID );
		flag.m_Instance.ZUM_DetailsBar:SetVoid2( unitID );
		flag.m_Instance.ZUM_DetailsBar:RegisterCallback( Mouse.eLClick, OnZumFlagSettingsClick );

		if pUnit ~= nil and unitInfo ~= nil then
			flag.m_Instance.ZUM_DetailsBar:SetHide( false );
			
			local isPanelOnTop = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, SETTINGS_NAME_PREFIX .. "_PANEL_ON_TOP" );
			if isPanelOnTop then
				--print( " PanelOnTop " );
				flag.m_Instance.ZUM_DetailsBar:SetOffsetY( PANEL_ON_TOP_OFFSET_Y );
			else
				flag.m_Instance.ZUM_DetailsBar:SetOffsetY( PANEL_ON_BOTTOM_OFFSET_Y );
			end

			local nothingShown = true; -- true if nothing shown on the panel and placeholder need to be shown
			
			--flag.m_Instance.ZUM_PaddingPrefix:SetHide( true );
			flag.m_Instance.ZUM_PaddingSuffix:SetHide( true );

			-- ==========================
			-- Strength icon
			-- ==========================
			local relStrength			= 0 ; -- religious 
			local melStrength			= pUnit:GetCombat(); -- melee
			local ranStrength			= pUnit:GetRangedCombat(); -- ranged
			local bomStrength			= pUnit:GetBombardCombat(); -- bombard

			if pUnit.GetReligiousStrength ~= nil then
				relStrength = pUnit:GetReligiousStrength();
			end

			flag.m_Instance.ZUM_MeleIcon:SetHide( true );
			flag.m_Instance.ZUM_RangedIcon:SetHide( true );
			flag.m_Instance.ZUM_BombardIcon:SetHide( true );
			flag.m_Instance.ZUM_ReligionIcon:SetHide( true );
			flag.m_Instance.ZUM_AttackNumber:SetHide( true );

			local settingIconColor = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, SETTINGS_NAME_PREFIX .. "_ATTACK_ICON" );
			local settingTextColor = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, SETTINGS_NAME_PREFIX .. "_ATTACK_ICON" );
			if ranStrength > 0 then
				if settingIconColor ~= CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_RangedIcon:SetHide( false );
					flag.m_Instance.ZUM_RangedIcon:SetColor( settingIconColor );
					nothingShown = false;
				end
				--flag.m_Instance.ZUM_AttackNumber:SetHide( false );
				--flag.m_Instance.ZUM_AttackNumber:SetText( ranStrength );
			elseif bomStrength > 0 then
				if settingIconColor ~= CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_BombardIcon:SetHide( false );
					flag.m_Instance.ZUM_BombardIcon:SetColor( settingIconColor );
					nothingShown = false;
				end
				--flag.m_Instance.ZUM_AttackNumber:SetHide( false );
				--flag.m_Instance.ZUM_AttackNumber:SetText( bomStrength );
			elseif relStrength > 0 then
				if settingIconColor ~= CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_ReligionIcon:SetHide( false );
					flag.m_Instance.ZUM_ReligionIcon:SetColor( settingIconColor );
					nothingShown = false;
				end
				--flag.m_Instance.ZUM_AttackNumber:SetHide( false );
				--flag.m_Instance.ZUM_AttackNumber:SetText( relStrength );
			elseif melStrength > 0 then
				if settingIconColor ~= CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_MeleIcon:SetHide( false );
					flag.m_Instance.ZUM_MeleIcon:SetColor( settingIconColor );
					nothingShown = false;
				end
				--flag.m_Instance.ZUM_AttackNumber:SetHide( false );
				--flag.m_Instance.ZUM_AttackNumber:SetText( melStrength );
			end

			-- ==========================
			-- Charges icon
			-- ==========================
			local buildCharges			= pUnit:GetBuildCharges(); -- build charges
			local spreadCharges			= 0; -- religious charges
			local healCharges			= 0; -- religious heal charges
				
			if pUnit.GetSpreadCharges ~= nil then
				spreadCharges = pUnit:GetSpreadCharges();
			end
			if pUnit.GetReligiousHealCharges ~= nil then
				healCharges = pUnit:GetReligiousHealCharges();
			end

			flag.m_Instance.ZUM_BuildChargeIcon:SetHide( true );
			flag.m_Instance.ZUM_ReligionChargeIcon:SetHide( true );
			flag.m_Instance.ZUM_HealChargeIcon:SetHide( true );
			flag.m_Instance.ZUM_ChargeNumber:SetHide( true );
			
			settingIconColor = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, SETTINGS_NAME_PREFIX .. "_CHARGES_ICON" );
			settingTextColor = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, SETTINGS_NAME_PREFIX .. "_CHARGES_TEXT" );
			
			local chargesShown = false;
			flag.m_Instance.ZUM_PaddingCharge:SetHide( nothingShown );
			if spreadCharges > 0 then
				if settingIconColor ~= CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_ReligionChargeIcon:SetHide( false );
					flag.m_Instance.ZUM_ReligionChargeIcon:SetColor( settingIconColor );
					nothingShown = false;
					chargesShown = true;
				end
				if settingTextColor ~= CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_ChargeNumber:SetHide( false );
					flag.m_Instance.ZUM_ChargeNumber:SetText( spreadCharges );
					flag.m_Instance.ZUM_ChargeNumber:SetColor( settingTextColor );
					flag.m_Instance.ZUM_PaddingSuffix:SetHide( false );
					nothingShown = false;
					chargesShown = true;
				end
			elseif healCharges > 0 then
				if settingIconColor ~= CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_HealChargeIcon:SetHide( false );
					flag.m_Instance.ZUM_HealChargeIcon:SetColor( settingIconColor );
					nothingShown = false;
					chargesShown = true;
				end
				if settingTextColor ~= CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_ChargeNumber:SetHide( false );
					flag.m_Instance.ZUM_ChargeNumber:SetText( healCharges );
					flag.m_Instance.ZUM_ChargeNumber:SetColor( settingTextColor );
					flag.m_Instance.ZUM_PaddingSuffix:SetHide( false );
					nothingShown = false;
					chargesShown = true;
				end
			elseif buildCharges > 0 then
				if settingIconColor ~= CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_BuildChargeIcon:SetHide( false );
					flag.m_Instance.ZUM_BuildChargeIcon:SetColor( settingIconColor );
					nothingShown = false;
					chargesShown = true;
				end
				if settingTextColor ~= CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_ChargeNumber:SetHide( false );
					flag.m_Instance.ZUM_ChargeNumber:SetText( buildCharges );
					flag.m_Instance.ZUM_ChargeNumber:SetColor( settingTextColor );
					flag.m_Instance.ZUM_PaddingSuffix:SetHide( false );
					nothingShown = false;
					chargesShown = true;
				end
			end -- if spreadCharges > 0
			if not chargesShown then
				flag.m_Instance.ZUM_PaddingCharge:SetHide( true );
			end
			
			-- ==========================
			-- Spy Text
			-- ==========================			
			flag.m_Instance.ZUM_PaddingPrefixSpy:SetHide( true );
			flag.m_Instance.ZUM_PaddingSuffixSpy:SetHide( true );

			local hideSpyText:boolean = true;
			
			flag.m_Instance.ZUM_SpyTurns:SetHide( true );

			settingTextColor = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, SETTINGS_NAME_PREFIX .. "_SPY_MISSION_TURNS" );

			if unitInfo.Spy == true and settingTextColor ~= CONST_COLOR_HIDE then
				local spyTurnsRemaining = pUnit:GetSpyOperationEndTurn() - Game.GetCurrentGameTurn();
				if spyTurnsRemaining > 0 then
					flag.m_Instance.ZUM_SpyTurns:SetText( spyTurnsRemaining );
					flag.m_Instance.ZUM_SpyTurns:SetToolTipString( Locale.Lookup("LOC_UNITPANEL_ESPIONAGE_MORE_TURNS", spyTurnsRemaining ) );
					
					flag.m_Instance.ZUM_SpyTurns:SetColor( settingTextColor );
					flag.m_Instance.ZUM_SpyTurns:SetHide( false );
					if spyTurnsRemaining < 10 then
						flag.m_Instance.ZUM_PaddingPrefixSpy:SetHide( false );
						flag.m_Instance.ZUM_PaddingSuffixSpy:SetHide( false );
					end

					hideSpyText = false;
					nothingShown = false;
				end -- if spyTurnsRemaining > 0 then
			end -- if unitInfo.Spy == true and settingTextColor ~= CONST_COLOR_HIDE 
			...
			flag.m_Instance.ZUM_TradeYields1:SetHide( true );
			flag.m_Instance.ZUM_TradeYields2:SetHide( true );
			flag.m_Instance.ZUM_TradeYields3:SetHide( true );
			flag.m_Instance.ZUM_TradeYields4:SetHide( true );
			flag.m_Instance.ZUM_TradeYields5:SetHide( true );
			flag.m_Instance.ZUM_DetailsTradeStack:SetHide( true );

			settingIconColor = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, SETTINGS_NAME_PREFIX .. "_TRADE_YIELDS" );

			if unitInfo.MakeTradeRoute == true and settingIconColor ~= CONST_COLOR_HIDE then
				local originPlayer:table = Players[ playerID ];
				local originCities:table = originPlayer:GetCities();
				local tradeRouteYields = "";
				local tradeRouteYieldIcons = {};
						...
						if unitID == route.TraderUnitID then
							...
							for j, yieldInfo in pairs( route.OriginYields ) do
								if yieldInfo.Amount > 0 then
									...
									local icon:string, text:string = FormatTradeYieldText( yieldDetails, Round( yieldInfo.Amount, 1 ) );
									if not hideTradeYields then
										-- if not the first yield then
										tradeRouteYields = tradeRouteYields .. " ";
									end
									tradeRouteYields = tradeRouteYields .. icon .. text;
									table.insert( tradeRouteYieldIcons, icon );
									--tradeRouteYieldIcons = tradeRouteYieldIcons .. icon;
									hideTradeYields = false;
								end -- if yieldInfo.Amount
							end -- for j, yieldInfo
						end -- if unitID
					end -- for i, route
				end -- for _, city

				if not hideTradeYields and #tradeRouteYieldIcons > 0 then

					flag.m_Instance.ZUM_DetailsTradeStack:SetHide( false );
					nothingShown = false;

					for i, icon in ipairs( tradeRouteYieldIcons ) do
						if i < 6 then
							local uiYieldLabel = flag.m_Instance["ZUM_TradeYields" .. i];
							uiYieldLabel:SetHide( false );
							uiYieldLabel:SetText( icon );
							uiYieldLabel:SetToolTipString( tradeRouteYields );
						end -- if i < 6
					end -- for i, icon 
				end -- if not hideTradeYields and #tradeRouteYieldIcons > 0 

			end -- if unitInfo.MakeTradeRoute == true and settingIconColor ~= CONST_COLOR_HIDE

			-- ==========================
			-- Modifiers icon
			-- ==========================
			-- @DEPRECATED replaced with abilities
			--local UnitModifiersList;
			--local isGreatPerson;
			--local isXpOnlyModifiers;
--
			--UnitModifiersList, isGreatPerson, isXpOnlyModifiers = QuickRetrieveUnitModifiers( playerID, unitID ); -- list of localized text entries
			--flag.m_Instance.ZUM_ModifiersIcon:SetHide( true );
			--flag.m_Instance.ZUM_ModifiersIconGP:SetHide( true );
			--if playerID == 0 then
				----print("                   " );
				----print("                   " );
				----print("                   Unit modifiers: ", playerID, unitID, #UnitModifiersList );
				----tprint( UnitModifiersList, 3 );
			--end
			--
			--settingIconColor = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, SETTINGS_NAME_PREFIX .. "_MODIFIERS_ICON" );
			--settingTextColor = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, SETTINGS_NAME_PREFIX .. "_MODIFIERS_GREAT" );
			--local hideXpOnly = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, SETTINGS_NAME_PREFIX .. "_MODIFIERS_HIDE_XP_ONLY" );
--
			--local modifiersShown = false;
			--flag.m_Instance.ZUM_PaddingModifier:SetHide( nothingShown );
			--local uiModifierIcon = flag.m_Instance.ZUM_ModifiersIcon;
--
			---- Great Person custom icon
			--if unitInfo.UnitType == "UNIT_GREAT_GENERAL" or unitInfo.UnitType == "UNIT_GREAT_ADMIRAL" then
				--uiModifierIcon = flag.m_Instance.ZUM_ModifiersIconGP;
			--end
--
			--if unitInfo.MakeTradeRoute ~= true and UnitModifiersList ~= nil and #UnitModifiersList > 0 and ( hideXpOnly == false or isXpOnlyModifiers == false ) then -- TODO: isXpOnlyModifiers
				--local isGreatPersonAffected = isGreatPerson; 
				--local tt = Locale.Lookup("LOC_ZUM_ACTIVE_UNIT_MODIFIERS") .. "[NEWLINE]" ;
				--for i, str in pairs( UnitModifiersList ) do
					--if str ~= nil and #str > 0 then
						--tt = tt .. str .. "[NEWLINE]";
					--end -- if str ~= nil 
				--end -- for i, str 
				--if isGreatPersonAffected and settingTextColor ~= CONST_COLOR_HIDE then
					--uiModifierIcon:SetHide( false );
					--uiModifierIcon:SetToolTipString( tt );
					--uiModifierIcon:SetColor( settingTextColor );
					--nothingShown = false;
					--modifiersShown = true;
				--elseif (not isGreatPersonAffected) and settingIconColor ~= CONST_COLOR_HIDE then
					--uiModifierIcon:SetHide( false );
					--uiModifierIcon:SetToolTipString( tt );
					--uiModifierIcon:SetColor( settingIconColor );
					--nothingShown = false;
					--modifiersShown = true;
				--end
--
			--end -- if UnitModifiersList ~= nil
			-- @DEPRECATED END
			...
			if ( unitAbilitiesList ~= nil and table.count( unitAbilitiesList ) > 0 ) then
				...
				for i,ability in ipairs ( unitAbilitiesList ) do
					...
					if ( abilityDef ~= nil ) then
						if ( abilityDef.Description ~= nil and 
-- DB
							( zumGrantedAbilities[ abilityDef.UnitAbilityType ] ~= nil or string.find( abilityDef.UnitAbilityType, "GREAT_ADMIRAL" ) or string.find( abilityDef.UnitAbilityType, "GREAT_GENERAL" ) or (GameConfiguration.GetValue("DB_SOLI_DEO_GLORIA") and string.find(abilityDef.UnitAbilityType, "GREAT_PROPHET")) )
-- /DB
							) then
							--abilityText = abilityText .. "[NEWLINE][ICON_Bullet] " .. Locale.Lookup(abilityDef.Description);
-- DB
							if ( string.find( abilityDef.UnitAbilityType, "GREAT_ADMIRAL" ) or string.find( abilityDef.UnitAbilityType, "GREAT_GENERAL" )  or (GameConfiguration.GetValue("DB_SOLI_DEO_GLORIA") and string.find(abilityDef.UnitAbilityType, "GREAT_PROPHET")) ) then
-- /DB
								isGreatPerson = true;
								table.insert( UnitModifiersList, 1, Locale.Lookup( "LOC_ZUM_GREATPERSON_ABILITY_PREFIX" ) .. Locale.Lookup( abilityDef.Description ) );
							else
								table.insert( UnitModifiersList, Locale.Lookup( abilityDef.Description ) );
							end
							if ( not string.find( abilityDef.UnitAbilityType, "_XP" ) ) then
								isXpOnlyModifiers = false;
							end
						end
					end
				end
			...
			-- Great Person custom icon
			local gpCustomDesc = nil;
			local pUnitGreatPerson:table = pUnit:GetGreatPerson();
			if pUnitGreatPerson ~= nil and pUnitGreatPerson:IsGreatPerson() and pUnitGreatPerson:HasPassiveEffect() then
			--if unitInfo.UnitType == "UNIT_GREAT_GENERAL" or unitInfo.UnitType == "UNIT_GREAT_ADMIRAL" then
				isGreatPerson = true;
				uiModifierIcon = flag.m_Instance.ZUM_ModifiersIconGP;
				gpCustomDesc = pUnitGreatPerson:GetPassiveEffectText() ; --Locale.Lookup( " " );
				--end
			end

			if unitInfo.MakeTradeRoute ~= true and UnitModifiersList ~= nil and ( #UnitModifiersList > 0 or gpCustomDesc ~= nil) and ( hideXpOnly == false or isXpOnlyModifiers == false ) then -- TODO: isXpOnlyModifiers
				local isGreatPersonAffected = isGreatPerson; 
				local tt = "";
				if gpCustomDesc ~= nil and gpCustomDesc ~= "" then
					tt = gpCustomDesc;
				else
					tt = Locale.Lookup("LOC_ZUM_ACTIVE_UNIT_MODIFIERS");
					for i, str in pairs( UnitModifiersList ) do
						if str ~= nil then
							tt = tt .. "[NEWLINE][ICON_Bullet]" .. str;
						end -- if str ~= nil 
					end -- for i, str 
				end -- if gpCustomDesc ~= nil
			...
			end -- if unitInfo.MakeTradeRoute ~= true and UnitModifiersList ~= nil

		


			if not modifiersShown then
				flag.m_Instance.ZUM_PaddingModifier:SetHide( true );
			end
			...
			flag.m_Instance.ZUM_PaddingCustomMark:SetHide( nothingShown );

			markIcon, markText, markIconColor, markTextColor = ExposedMembers.ZUM.GetMark( Game.GetLocalPlayer(), playerID, unitID );
			if markIcon ~= nil and markIconColor ~= nil and markTextColor ~= nil then
				
				if markText == nil then
					markText = "";
				end
				flag.m_Instance.ZUM_CustomMark:SetText( markText );
				flag.m_Instance.ZUM_CustomMarkIcon:SetIcon( markIcon );

				flag.m_Instance.ZUM_CustomMarkIcon:SetColor( markIconColor );
				flag.m_Instance.ZUM_CustomMark:SetColor( markTextColor );
				--print( "Unit flag mark updated: ", playerID, unitID );

				if markIconColor == CONST_COLOR_HIDE then
					flag.m_Instance.ZUM_CustomMarkIcon:SetHide( true );
				else
					flag.m_Instance.ZUM_CustomMarkIcon:SetHide( false );
					markShown = true;
					nothingShown = false;
				end
				if markTextColor == CONST_COLOR_HIDE or markText == "" then
					flag.m_Instance.ZUM_CustomMark:SetHide( true );
				else
					flag.m_Instance.ZUM_CustomMark:SetHide( false );
					flag.m_Instance.ZUM_PaddingSuffix:SetHide( false );
					markShown = true;
					nothingShown = false;
				end
				...
				flag.m_Instance.ZUM_CustomMarkIcon:SetHide( true );
				flag.m_Instance.ZUM_CustomMark:SetHide( true );
				--print( "Unit flag mark hiden: ", playerID, unitID );
			end

			if not markShown then
				flag.m_Instance.ZUM_PaddingCustomMark:SetHide( true );
			end
			...
			local hideEmptyPanel = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, "HIDE_EMPTY_PANEL" );
			if nothingShown then
				if not hideEmptyPanel then
					flag.m_Instance.ZUM_DetailsBar:SetToolTipString( Locale.Lookup("LOC_ZUM_NOTHING_TO_SHOW_TT") );
					flag.m_Instance.ZUM_PaddingSuffix:SetHide( true );
					flag.m_Instance.ZUM_EmptyPlaceholder:SetHide( false );
					flag.m_Instance.ZUM_DetailsBar:SetHide( false );
				else
					flag.m_Instance.ZUM_DetailsBar:SetToolTipString( Locale.Lookup("LOC_ZUM_NOTHING_TO_SHOW_TT") );
					flag.m_Instance.ZUM_PaddingSuffix:SetHide( true );
					flag.m_Instance.ZUM_EmptyPlaceholder:SetHide( true );
					flag.m_Instance.ZUM_DetailsBar:SetHide( true );
				end
			else
				flag.m_Instance.ZUM_DetailsBar:SetToolTipString( nil );
				flag.m_Instance.ZUM_PaddingSuffix:SetHide( false );
				flag.m_Instance.ZUM_EmptyPlaceholder:SetHide( true );
				flag.m_Instance.ZUM_DetailsBar:SetHide( false );
			end
...
--************************************************************
function ZumUpdateTopBarForAllUnits()
	for player_num = 0, PlayerManager.GetWasEverAliveCount() - 1 do
		local player = Players[ player_num ];
		if player:WasEverAlive() then
			for _,pUnit : object in player:GetUnits():Members() do
				if not pUnit:IsDead() then
					ZumUpdateTopBar( player:GetID(), pUnit:GetID() );
				end -- if not pUnit:IsDead()
			end -- for _,pUnit
		end -- if player:WasEverAlive()
	end -- for player_num = 0
end

--************************************************************
function OnUnitChargesChanged( playerID : number, unitID : number )
	ZumUpdateTopBar( playerID, unitID ); -- Zur13
end
...
function OnZumFlagSettingsClick( playerID : number, unitID : number )
	...
	if isClickAllowed then
		...
		ZumUpdateTopBar( playerID, unitID ); -- Zur13
			...
			--local UnitModifiersList;
			--local isGreatPerson;
			--UnitModifiersList, isGreatPerson = QuickRetrieveUnitModifiers( playerID, unitID, true ); -- list of localized text entries
			--
			--print("                   " );
			--print("                   " );
			--print("                   Unit modifiers: ", playerID, unitID, #UnitModifiersList, isGreatPerson );
			--
			--
			--if UnitModifiersList ~= nil and #UnitModifiersList > 0 then
				--local tt = "";
				--for i, str in pairs( UnitModifiersList ) do
					--if str ~= nil and #str > 0 then
						--print( " modifier: ", str );
...
--************************************************************
-- Update AOE modifiers for all player combat units
function ZumUpdatePlayerUnits( playerID )
	local pPlayer:table = Players[ playerID ];
	local pPlayerUnits:table = pPlayer:GetUnits();
	for i, pUnit in pPlayerUnits:Members() do
		if pUnit:GetCombat() > 0 or pUnit:GetRangedCombat() > 0 or pUnit:GetAntiAirCombat() > 0 or pUnit:GetBombardCombat() > 0 then
			ZumUpdateTopBar( playerID, pUnit:GetID() ); -- Zur13
		end
	end
end

--************************************************************
-- Update AOE modifiers for all player units in range
function ZumUpdatePlayerUnitsInRange( playerID, x, y )
	local pPlayer:table = Players[ playerID ];
	local pPlayerUnits:table = pPlayer:GetUnits();
	for i, pUnit in pPlayerUnits:Members() do
		if pUnit:GetCombat() > 0 or pUnit:GetRangedCombat() > 0 or pUnit:GetAntiAirCombat() > 0 or pUnit:GetBombardCombat() > 0 then
			local distanceToGreatPerson:number = Map.GetPlotDistance( pUnit:GetX(), pUnit:GetY(), x, y );
			-- need to update surrounding units in move start location and move end location
			-- base moves 4 * railroad 0.25 = 16 max
			if distanceToGreatPerson < 20 then 
				ZumUpdateTopBar( playerID, pUnit:GetID() ); -- Zur13
			end
		end
	end
...
--************************************************************
-- Update local player spy units
function ZumUpdatePlayerSpies( )
	local playerID = Game.GetLocalPlayer();
	local pPlayer:table = Players[ playerID ];
	local pPlayerUnits:table = pPlayer:GetUnits();
	for i, pUnit in pPlayerUnits:Members() do
		local unitInfo = GameInfo.Units[ pUnit:GetType() ];
		if unitInfo ~= nil and unitInfo.Spy == true then
			ZumUpdateTopBar( playerID, pUnit:GetID() ); -- Zur13
		end
	end
end
...
--************************************************************
function ZumOnUnitMoved( playerID:number, unitID:number )
	local kUnit = Players[ playerID ]:GetUnits():FindID( unitID );
	if kUnit ~= nil then
		local kUnitGreatPerson:table = kUnit:GetGreatPerson();
		if kUnitGreatPerson ~= nil and kUnitGreatPerson:IsGreatPerson() then
			--print( "ZumOnUnitMoved( playerID:number, unitID:number )", playerID, unitID );
			ZumUpdatePlayerUnitsInRange( playerID, kUnit:GetX(), kUnit:GetY() );
		end -- if kUnitGreatPerson ~= nil 
	end -- if kUnit ~= nil 

end

--************************************************************
function ZumOnUnitCommandStarted( playerID, unitID, hCommand, iData1 )
	--if ( hCommand == GameInfo.Types["UNITCOMMAND_NAME_UNIT"].Hash ) then
		--local flagInstance = GetUnitFlag( playerID, unitID );
		--if (flagInstance ~= nil) then
			--flagInstance:UpdateName();
	...
	if kUnit ~= nil then
		local kUnitGreatPerson:table = kUnit:GetGreatPerson();
		if kUnitGreatPerson ~= nil and kUnitGreatPerson:IsGreatPerson() then
			ZumUpdatePlayerUnits( playerID );
		end -- if kUnitGreatPerson ~= nil 
	end -- if kUnit ~= nil 
end

--************************************************************
function ZumOnUnitOperationStarted( playerID:number, unitID:number, operationID:number )
	--if m_AnimSupport.IsVisible() and operationID == UnitOperationTypes.MAKE_TRADE_ROUTE then
		--Refresh();
	--end
	--print( "ZumOnUnitOperationStarted()", playerID, unitID, operationID );
	local kUnit = Players[ playerID ]:GetUnits():FindID( unitID );
	if kUnit ~= nil then
		local kUnitGreatPerson:table = kUnit:GetGreatPerson();
		if kUnitGreatPerson ~= nil and kUnitGreatPerson:IsGreatPerson() then
		...
		if unitInfo ~= nil and unitInfo.Spy == true and Controls.SpyMissionUpdateDelayTimer ~= nil then
			Controls.SpyMissionUpdateDelayTimer:Stop();
			Controls.SpyMissionUpdateDelayTimer:SetToBeginning();
			Controls.SpyMissionUpdateDelayTimer:Play();
		end
	end -- if kUnit ~= nil 
end
...
function OnSpyMissionUpdated( ... )
	--printResult = "OnSpyMissionUpdated( ";
...
include( "ZUM_HighlightUnitPathOnHover" );

--************************************************************
function ZUM_OnUnitFlagPointerEntered( playerID:number, unitID:number )
	local localPlayer = Game.GetLocalPlayer()
	if playerID == localPlayer then
		if playerID ~= -1 and Players[ playerID ] ~= nil  and m_UnitFlagInstances[ playerID ] ~= nil then
			local flag = GetUnitFlag( playerID, unitID );
    ...
    if Zum_ShowPathsOnHover and not Zum_IsPathShown then
		local idLocalPlayer = Game.GetLocalPlayer();
		local showPathOnHover = ExposedMembers.ZUM.GetSettingColor( idLocalPlayer, "PATH_ON_FLAG_HOVER" );
		if not showPathOnHover then
			return;
		end

		-- @ Zur13 flag mouse over may happen when active interface mode ~= InterfaceModeTypes.SELECTION
		local uiMode = UI.GetInterfaceMode();
		if uiMode ~= InterfaceModeTypes.SELECTION then
			return;
		end

		-- @ Zur13 do nothing if some unit is selected
		local selUnit = UI.GetHeadSelectedUnit();
		if selUnit ~= nil then
			return;
		end

		-- @ Zur13 do nothing if other player's unit flag
		if playerID ~= Game.GetLocalPlayer() then
			return;
		end
        
        RealizeMovementPath( true, unitID );
		Zum_IsPathShown = true;
    end
end

--************************************************************
function ZUM_OnUnitFlagPointerExited( playerID:number, unitID:number )
    if Zum_IsPathShown then
		Zum_IsPathShown = false;
		-- @ Zur13 do nothing if some unit is selected
		local selUnit = UI.GetHeadSelectedUnit();
		if selUnit ~= nil then
			return;
...
--************************************************************
function ZUM_OnUnitPromotionAvailable( playerID:number, unitID:number, promotion )
    local localPlayer = Game.GetLocalPlayer()
	if playerID == localPlayer then
		if playerID == -1 then
			return;
...
--************************************************************
function ZumInitialize(  playerID : number, unitID : number  )
	if ExposedMembers.ZUM == nil then
		ExposedMembers.ZUM = {};
	end
	ExposedMembers.ZUM.ZumUpdateTopBar = ZumUpdateTopBar;
	ExposedMembers.ZUM.ZumUpdateTopBarForAllUnits = ZumUpdateTopBarForAllUnits;

	Events.UnitChargesChanged.Add( OnUnitChargesChanged );

	Events.UnitCommandStarted.Add( ZumOnUnitCommandStarted );
	Events.UnitOperationStarted.Add( ZumOnUnitOperationStarted );

	Events.SpyMissionUpdated.Add( OnSpyMissionUpdated );
	Events.SpyUpdated.Add( OnSpyUpdated );
	Controls.SpyMissionUpdateDelayTimer:RegisterEndCallback( ZumUpdatePlayerSpies );
	Events.LocalPlayerTurnBegin.Add( ZumUpdatePlayerSpies );

-- DB
	Events.UnitMoved.Add(OnUnitArtifactChanged)
-- /DB
	--Events.UnitMoved.Add( ZumOnUnitMoved );
	Events.UnitAbilityLost.Add( ZumUpdateTopBar );

	Events.UnitPromotionAvailable.Add(	ZUM_OnUnitPromotionAvailable );
	Events.UnitPromoted.Add( ZUM_OnUnitPromotionAvailable );


    LuaEvents.UnitFlagManager_PointerEntered.Add( ZUM_OnUnitFlagPointerEntered );
    LuaEvents.UnitFlagManager_PointerExited.Add( ZUM_OnUnitFlagPointerExited );
end
