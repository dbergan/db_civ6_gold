...
-- DB - don't add tabs for other players [Remove Foreign Information]
			if(not GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") and religionType ~= m_PlayerReligionType) then
-- /DB
...
-- DB - remove the all religions tab button [Remove Foreign Information] or remove the "3/9" label on the all religions tab button [SDG]
		if not GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") then 
			if GameConfiguration.GetValue("DB_SOLI_DEO_GLORIA") then  -- SDG
...
-- DB - allow changing of religion names [SDG: All Religions Can Have Custom Names]
				if GameConfiguration.GetValue("DB_ALL_CUSTOM_NAMES") then
					Controls.ConfirmReligion:SetDisabled(true)
