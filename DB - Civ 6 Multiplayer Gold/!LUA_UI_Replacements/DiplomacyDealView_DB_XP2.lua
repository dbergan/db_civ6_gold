...
-- DB		2021-03-12: line 445
		if GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") and ms_OtherPlayerIsHuman and DealDirection.OUTGOING then
			bDealValid = true;
		end
-- /DB
...
-- DB		2021-03-30: line 776 [DB_RFI: disable right increment arrow]
...
-- DB		2021-03-30: line 866 [DB_RFI: disable click-increments for the other player]
					if GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") and player:GetID() == ms_OtherPlayerID then
						return ;
					end
-- /DB
...
-- DB		2021-03-30: line 894 [DB_RFI: only add 1 to the other player's deal]
				if GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") and player:GetID() == ms_OtherPlayerID then
					iAddAmount = 1
				end
-- /DB
...
-- DB		2021-03-30: line 942 [DB_RFI: disable click-increments for the other player]
					if GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") and player:GetID() == ms_OtherPlayerID then
						return ;
					end
-- /DB
		...
		if (not bFound) then
			...
			if (pDealItem ~= nil) then
				...
				-- Adjust the gold to our max
-- DB		2021-03-30: line 970 [DB_RFI: only add 1 to the other player's deal]
				if GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") and player:GetID() == ms_OtherPlayerID then
					iAddAmount = 1
				end
-- /DB
				...
				if (not ms_OtherPlayerIsHuman) then
					...
					if (ms_LastIncomingDealProposalAction == DealProposalAction.PENDING) then
						...
						if (itemsFromLocal > 0 and itemsFromOther == 0) then
							...
							Controls.AcceptDeal:LocalizeAndSetText("LOC_DIPLOMACY_DEAL_GIFT_DEAL");
-- DB		2021-03-30: line 1135 [DB_RFI: control buttons and panels when local player hasn't offered anything]
						elseif GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") and (itemsFromLocal == 0 and itemsFromOther == 0) then
							g_uiMyOffers.Directions:SetHide(true);
...
-- DB		2021-03-30: line 1204 [DB_RFI: Human opponent doesn't tell you when a deal is "invalid"]
			if (GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") and not ms_OtherPlayerIsHuman) or (not GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO")) then
...
-- DB		2021-03-30: line 1218 [DB_RFI: Hide "Accept Deal" button when the deal isn't valid]
		if GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") and (not bDealValid) then
			Controls.AcceptDeal:SetHide(true);
		end 
-- /DB
		if (ms_bIsDemand) then
...
-- DB		2021-03-30: line 1297 [DB_RFI: Hide available gold]
					if GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") then
...
-- DB		2021-03-30: line 1731 [DB_RFI: clip values only for self, not opponent]
			if GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") and pDealItem:GetFromPlayerID() == g_LocalPlayer:GetID() then
...
-- DB		2021-03-30: line 1785 [DB_RFI: Hide amount]
					if GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") then
						uiIcon.AmountText:SetHide(true);
					else
						uiIcon.AmountText:SetHide(false);
...
function PopulateAvailableCities(player : table, iconList : table)
	...
	local uiMinimizedSection : table = ms_MinimizedSectionIM:GetInstance(iconList.List);
-- DB
	if GameConfiguration.GetValue("DB_NO_CITY_TRADES") then
		possibleItems = nil
	end
-- /DB
...
-- DB		2021-03-30: line 2009 [DB_RFI: Hide city tooltips]
				if not GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") then
...
-- DB		2021-03-30: line 2698 [DB_RFI: hide city tooltips]
					if not GameConfiguration.GetValue("DB_REMOVE_FOREIGN_INFO") then
