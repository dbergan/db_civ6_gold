...
function group_military( unit, unitInstance, group, parent, type )
	...
	for _,mod in ipairs(tUnitModifiers) do
		...
		elseif tMod.EffectType == "EFFECT_ADJUST_UNIT_FLANKING_BONUS_MODIFIER" then
			AddExtraPromoText( Locale.Lookup("LOC_COMBAT_PREVIEW_FLANKING_BONUS_DESCRIPTION", tMod.Arguments.Percent.."%") ); -- +x%
		...
		elseif tMod.EffectType == "EFFECT_ADJUST_UNIT_SUPPORT_BONUS_MODIFIER" then
			AddExtraPromoText( Locale.Lookup("LOC_COMBAT_PREVIEW_SUPPORT_BONUS_DESCRIPTION", tonumber(tMod.Arguments.Percent)) ); -- +x% support bonus
		else
...
-- DB
	if GameConfiguration.GetValue("DB_SOLI_DEO_GLORIA") then
